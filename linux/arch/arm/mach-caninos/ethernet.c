...
{
    INFO_MSG(" ethernet_put_pin_mux(void)");
	if (ethernet_ppc)
	{
...
{
    INFO_MSG(" ethernet_put_clk(void)");
    if (ethernet_clk)
...
{
	int ret;
    INFO_MSG(" ethernet_clock_enable(int reset)");
...
static int ethernet_mac_init(void)
{
    ...
    INFO_MSG(" ethernet_mac_init(void)");
	...
	act_writel(EC_CACHETHR_CPTL(0x0) | EC_CACHETHR_CRTL(0x0) | EC_CACHETHR_PQT(0x4FFF), MAC_CSR18);
	...
	act_writel(act_readl(MAC_CSR6) | EC_OPMODE_SPEED(0), MAC_CSR6); //100M
...
/*static int ethernet_parse_gpio(struct device_node * of_node, const char * propname, struct ethernet_gpio * gpio)
{
    ...
    INFO_MSG(" ethernet_parse_gpio(struct device_node * of_node, const char * propname, struct ethernet_gpio * gpio)");
    int	gpio_num;
...
static int ethernet_write_phy_reg(u16 reg_addr, u16 val)
{
    ...
    INFO_MSG(" ethernet_write_phy_reg(u16 reg_addr, u16 val)");
...
{
	u16 reg_val;
    INFO_MSG(" ethernet_phy_reg_set_bits(u16 reg_addr, u16 bits)");
...
{
	u16 reg_val;
    INFO_MSG(" ethernet_phy_reg_clear_bits(u16 reg_addr, u16 bits)");
...
{
    u16 bmsr;
    INFO_MSG(" ethernet_phy_get_link(bool * linked)");
    int ret;
...
{
    u16 lpa, adv, bmcr;
    INFO_MSG(" ethernet_phy_read_status(bool * speed, bool * full)");
...
{
    u16 bmcr, eval;
    INFO_MSG(" ethernet_phy_set_mode(bool speed, bool duplex)");
    int ret;
...
{
    int ret;
    INFO_MSG(" ethernet_phy_start_autoneg(void)");
    u16 adv;
...
{
    u32 cnt, phy_id;
    INFO_MSG(" ethernet_phy_init(void)");
	u16 reg_val;
...
static int ethernet_dma_init(void)
{
    ...
    INFO_MSG(" ethernet_dma_init(void)");
...
static inline void raw_rx_buffer_init(void)
{
    ...
    INFO_MSG(" void raw_rx_buffer_init(void)");
    int i;
...
static inline void raw_tx_buffer_init(void)
{
    ...
    INFO_MSG(" void raw_tx_buffer_init(void)");
    int i;
...
{
    int i;
    INFO_MSG(" void free_rxtx_skbs(struct sk_buff **array, int len)");
...
static char * ethernet_build_setup_frame(char * buffer, int buf_len)
{
    ...
    INFO_MSG(" * ethernet_build_setup_frame(char * buffer, int buf_len)");
    char * frame = buffer;
    char * mac;
...
static int ethernet_transmit_setup_frame(void)
{
    ...
    INFO_MSG(" ethernet_transmit_setup_frame(void)");
    struct sk_buff * skb = NULL;
...
static irqreturn_t ethernet_isr(int irq, void *cookie)
{
    ...
    INFO_MSG(" ethernet_isr(int irq, void *cookie)");
...
{
    INFO_MSG(" ethernet_netdev_start_xmit(struct sk_buff * skb, ");
...
{
    unsigned long flags;
    INFO_MSG(" ethernet_netdev_open(struct net_device * _unused)");
    int ret = 0;
...
{
    unsigned long flags;
    INFO_MSG(" ethernet_netdev_close(struct net_device *dev)");
...
{
    int ret;
    INFO_MSG(" ethernet_mac_start(bool speed, bool duplex)");
...
{
    // stop tx and rx and disable interrupts
    INFO_MSG(" ethernet_mac_stop(void)");
    
    act_writel(act_readl(MAC_CSR6) & (~(EC_OPMODE_ST | EC_OPMODE_SR)), 
               MAC_CSR6);
    act_writel(0, MAC_CSR7);
...
static void ethernet_tx_buffer_free(void)
{
    ...
    INFO_MSG(" ethernet_tx_buffer_free(void)");
...
static void ethernet_rx_buffer_free(void)
{
    ...
    INFO_MSG(" ethernet_rx_buffer_free(void)");
...
{
    const u32 total = sizeof(struct ethernet_buffer_desc) * RX_RING_SIZE;
    INFO_MSG(" ethernet_rx_buffer_alloc(void)");
    dma_addr_t * paddr = &ethernet_rx_buf_paddr, tmp;
    struct device * dev = &ethernet_dev->dev;
    struct sk_buff * skb;
    int i;
...
static void ethernet_phy_state_worker(struct work_struct * _unused)
{
    ...
    INFO_MSG(" ethernet_phy_state_worker(struct work_struct * _unused)");
    unsigned long flags;
    int ret;
...
{
    int ret;
    INFO_MSG(" ethernet_netdev_init(struct net_device * _unused)");
...
{
    INFO_MSG(" ethernet_netdev_uninit(struct net_device * _unused)");
    return;
}
...
static int ethernet_netdev_set_mac_address(struct net_device *ndev, void *address)
{
    ...
    INFO_MSG(" ethernet_netdev_set_mac_address(struct net_device *ndev, void *address)");
...
{
    int ret, irq;
    INFO_MSG(" __init eth_driver_probe(struct platform_device * pdev)");
...
{
    INFO_MSG(" __exit eth_driver_remove(struct platform_device *pdev)");
    if (ethernet_dev)
    {
